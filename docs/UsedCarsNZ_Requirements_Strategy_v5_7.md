...
| **Prerequisite (v5.4)** | A **pilot agreement** covering: data-processing authorisation for forwarded buyer enquiries, AI-labelled-response authorisation under §7, and 30-day raw-email retention. **A founder-prepared draft scaffold now exists** (`docs/legal/pilot-agreement-DRAFT.md`, 19 July — with [LAWYER: …] markers on every clause needing judgment, and Schedule A carrying the baseline options below), **but it is unreviewed and still blocks the pilot until the §14 item 6 consult signs it off.** |
